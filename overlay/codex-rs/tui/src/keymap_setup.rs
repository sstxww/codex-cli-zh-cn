...
fn keymap_action_menu_hint_line() -> Line<'static> {
    crate::i18n::tr_line(Line::from(vec![
        "enter".cyan(),
        " select · ".dim(),
        "esc".cyan(),
        " back".dim(),
...
pub(crate) fn build_keymap_action_menu_params(
    ...
    let context_label = descriptor
        ...
        .to_string();
    let upstream_description = descriptor
    ...
    let description = if crate::i18n::is_zh_cn() {
        format!("配置“{label}”操作的快捷键。")
    } else {
        upstream_description.to_string()
    };
    ...
    let source = if custom_binding {
        crate::i18n::tr_static("Custom root override").cyan()
    } else {
        crate::i18n::tr_static("Default keymap").dim()
    };
    let mut header = ColumnRenderable::new();
    header.push(Line::from(crate::i18n::tr_static("Edit Shortcut").bold()));
    header.push(Line::from(vec![
        label.bold(),
        " · ".dim(),
        crate::i18n::tr_owned(context_label).dim(),
    ]));
    header.push(Line::from(vec![
        crate::i18n::tr_static("Current ").dim(),
        key_binding_span(&current_binding),
        " · ".dim(),
        source,
    ]));
    header.push(Line::from(vec![
        crate::i18n::tr_static("Config ").dim(),
