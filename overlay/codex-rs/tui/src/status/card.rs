    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                crate::i18n::tr_owned("API key configured (run codex login to use ChatGPT)")
            }
        });
        ...
        let note_first_line = Line::from(vec![
            Span::from(crate::i18n::tr_static("Visit ")).cyan(),
            CHATGPT_USAGE_URL.cyan().underlined(),
            Span::from(crate::i18n::tr_static(" for up-to-date")).cyan(),
        ]);
        let note_second_line = Line::from(vec![
            Span::from(crate::i18n::tr_static(
        ...
        lines.push(formatter.line(
            "Permissions",
            vec![Span::from(crate::i18n::tr_owned(&self.permissions))],
        ));
        ...
        if let Some(collab_mode) = self.collaboration_mode.as_ref() {
            lines.push(formatter.line(
                "Collaboration mode",
                vec![Span::from(crate::i18n::tr_owned(collab_mode))],
            ));
        }
