...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        let help_lines = help_lines.into_iter().map(crate::i18n::tr_line).collect();
        ...
        if requested_model != session.model.as_str() {
            let lines = vec![
                crate::i18n::tr_static("model changed:")
                    .magenta()
                    .bold()
                    .into(),
                format!("{}{requested_model}", crate::i18n::tr_static("requested: ")).into(),
                format!("{}{}", crate::i18n::tr_static("used: "), session.model).into(),
            ];
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let change_model_hint_explanation = crate::i18n::tr_static(" to change");
        let dir_label_text = crate::i18n::tr_static("directory:");
        let permissions_label_text = crate::i18n::tr_static("permissions:");
        let model_label_text = crate::i18n::tr_static("model:");
        let label_width = if self.yolo_mode {
            display_width(dir_label_text).max(display_width(permissions_label_text))
        } else {
            display_width(dir_label_text).max(display_width(model_label_text))
        };
        ...
        let model_label = pad_label(model_label_text);
        ...
        let model_spans: Vec<Span<'static>> = {
            ...
            if let Some(reasoning) = reasoning_label {
                ...
                spans.push(Span::from(crate::i18n::tr_owned(reasoning)));
            }
            if self.show_fast_status {
                spans.push("   ".into());
                spans.push(Span::styled(
                    crate::i18n::tr_static("fast"),
                    self.model_style.magenta(),
                ));
            }
            spans.push("   ".dim());
        ...
        let dir_label = pad_label(dir_label_text);
        ...
        if self.yolo_mode {
            let permissions_label = pad_label(permissions_label_text);
            lines.push(make_row(vec![
                Span::from(format!("{permissions_label} ")).dim(),
                crate::i18n::tr_static("YOLO mode").magenta().bold(),
            ]));
        }
