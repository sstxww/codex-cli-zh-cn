...
impl HistoryCell for McpImageOutputCell {
    fn display_lines(&self, _width: u16) -> Vec<Line<'static>> {
        vec![crate::i18n::tr_static("tool result (image output)").into()]
    }
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    let mut docs_line = HyperlinkLine::new(Line::from(crate::i18n::tr_static("    See the ")));
    docs_line.push_span(
        crate::i18n::tr_static("MCP docs").underlined(),
        Some("https://developers.openai.com/codex/mcp"),
    );
    docs_line.push_span(
        crate::i18n::tr_static(" to configure them.").into(),
        /*destination*/ None,
    );
    ...
    let lines = vec![
        ...
        HyperlinkLine::from(""),
        HyperlinkLine::new(vec!["🔌  ".into(), crate::i18n::tr_static("MCP Tools").bold()].into()),
        HyperlinkLine::from(""),
        HyperlinkLine::new(
            crate::i18n::tr_static("  • No MCP servers configured.")
                .italic()
                .into(),
        ),
...
pub(crate) fn new_mcp_tools_output(
    ...
    let mut lines: Vec<Line<'static>> = vec![
        "/mcp".magenta().into(),
        "".into(),
        vec!["🔌  ".into(), crate::i18n::tr_static("MCP Tools").bold()].into(),
        "".into(),
    ];

    if tools.is_empty() {
        lines.push(
            crate::i18n::tr_static("  • No MCP tools available.")
                .italic()
                .into(),
        );
        lines.push("".into());
    ...
    for (server, cfg) in servers {
        ...
        if !cfg.enabled {
            header.push(" ".into());
            header.push(crate::i18n::tr_static("(disabled)").red());
            lines.push(header.into());
            if let Some(reason) = cfg.disabled_reason.as_ref().map(ToString::to_string) {
                lines.push(
                    vec![
                        crate::i18n::tr_static("    • Reason: ").into(),
                        reason.dim(),
                    ]
                    .into(),
                );
        ...
        lines.push(
            vec![
                crate::i18n::tr_static("    • Status: ").into(),
                crate::i18n::tr_static("enabled").green(),
            ]
            .into(),
        );
        lines.push(
            vec![
                crate::i18n::tr_static("    • Auth: ").into(),
        ...
        match &cfg.transport {
            McpServerTransportConfig::Stdio {
                ...
                lines.push(
                    vec![
                        crate::i18n::tr_static("    • Command: ").into(),
                        cmd_display.into(),
                    ]
                    .into(),
                );
            ...
            McpServerTransportConfig::StreamableHttp {
                    ...
                    && !headers.is_empty()
                {
                    ...
                    lines.push(
                        vec![
                            crate::i18n::tr_static("    • HTTP headers: ").into(),
                            display.into(),
                        ]
                        .into(),
                    );
                    ...
                    && !headers.is_empty()
                {
                    ...
                    lines.push(
                        vec![
                            crate::i18n::tr_static("    • Env HTTP headers: ").into(),
                            display.into(),
                        ]
                        .into(),
                    );
        ...
        if names.is_empty() {
            lines.push(crate::i18n::tr_static("    • Tools: (none)").into());
        } else {
            lines.push(
                vec![
                    crate::i18n::tr_static("    • Tools: ").into(),
                    names.join(", ").into(),
                ]
                .into(),
            );
        ...
        if server_resources.is_empty() {
            lines.push(crate::i18n::tr_static("    • Resources: (none)").into());
        } else {
            let mut spans: Vec<Span<'static>> =
                vec![crate::i18n::tr_static("    • Resources: ").into()];
        ...
        if server_templates.is_empty() {
            lines.push(crate::i18n::tr_static("    • Resource templates: (none)").into());
        } else {
            let mut spans: Vec<Span<'static>> =
                vec![crate::i18n::tr_static("    • Resource templates: ").into()];
    ...
    fn display_lines(&self, _width: u16) -> Vec<Line<'static>> {
        vec![
            vec![
                ...
                .unwrap_or_else(|| "•".dim()),
                " ".into(),
                crate::i18n::tr_static("Loading MCP inventory").bold(),
                "…".dim(),
            ]
            .into(),
        ]
