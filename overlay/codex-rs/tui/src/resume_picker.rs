...
impl SessionPickerAction {
    fn title(self) -> &'static str {
        crate::i18n::tr_static(match self {
...
fn sort_key_label(sort_key: ThreadSortKey) -> &'static str {
    crate::i18n::tr_static(match sort_key {
...
fn search_line(state: &PickerState, width: u16) -> Line<'_> {
    ...
    let search = if state.query.is_empty() {
        crate::i18n::tr_static("Type to search").dim()
    } else {
        format!("{}{}", crate::i18n::tr_static("Search: "), state.query).into()
    };
    ...
    if matches!(state.action, SessionPickerAction::Resume) {
        ...
        if compact {
            let active_status = match state.status {
                SessionStatus::Active => crate::i18n::tr_static("Active"),
                SessionStatus::Archived => crate::i18n::tr_static("Archived"),
            };
            ...
            spans.push(crate::i18n::tr_static("Status: ").dim());
            spans.push(toolbar_value(
                crate::i18n::tr_static("Active"),
                state.status == SessionStatus::Active,
                status_focused,
            ));
            spans.push(toolbar_value(
                crate::i18n::tr_static("Archived"),
...
fn filter_control_spans(state: &PickerState, compact: bool) -> Vec<Span<'static>> {
    ...
    if compact || state.filter_cwd.is_none() {
        return vec![
            crate::i18n::tr_static("Filter:").dim(),
            toolbar_value(
...
fn filter_mode_label(filter_mode: SessionFilterMode) -> &'static str {
    crate::i18n::tr_static(match filter_mode {
...
fn render_transcript_loading_overlay(frame: &mut crate::custom_terminal::Frame, area: Rect) {
    ...
    let message = crate::i18n::tr_static("Loading transcript…");
...
fn fit_footer_hint_refs(
    ...
    for (idx, hint) in hints.iter().enumerate() {
        ...
        if let Some(label) = label {
            ...
            spans.push(crate::i18n::tr_owned(label).set_style(footer_hint_label_style()));
        }
    }
    Some(spans.into())
}
            ...
            .map(|(idx, hint)| {
                let label_width = match mode {
                    FooterHintLabelMode::Wide => {
                        1 + UnicodeWidthStr::width(
                            crate::i18n::tr(hint.wide_label.as_str()).as_ref(),
                        )
                    }
                    FooterHintLabelMode::Compact => {
                        1 + UnicodeWidthStr::width(
                            crate::i18n::tr(hint.compact_label.as_str()).as_ref(),
                        )
                    }
...
fn render_empty_state_line(state: &PickerState) -> Line<'static> {
    if !state.query.is_empty() {
        ...
        {
            return vec![crate::i18n::tr_static("Searching…").italic().dim()].into();
        }
        ...
        return vec![
            crate::i18n::tr_static("No results for your search")
                .italic()
                .dim(),
        ]
        .into();
    ...
    if state.pagination.is_loading() {
        if state.all_rows.is_empty() && state.pagination.num_scanned_files == 0 {
            return vec![crate::i18n::tr_static("Loading sessions…").italic().dim()].into();
        }
        return vec![
            crate::i18n::tr_static("Loading older sessions…")
                .italic()
                .dim(),
        ]
        .into();
    ...
    vec![crate::i18n::tr_static("No sessions yet").italic().dim()].into()
}
