    ...
    pub(crate) fn new(
        ...
        if crate::i18n::is_zh_cn() {
            for item in &mut features {
                ...
                item.description = crate::i18n::tr_owned(&item.description);
            }
        }

        let mut header = ColumnRenderable::new();
        header.push(Line::from(
            crate::i18n::tr_static("Experimental features").bold(),
        ));
        header.push(Line::from(
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if list_area.height > 0 {
            ...
            render_rows(
                ...
                &self.state,
                MAX_POPUP_ROWS,
                crate::i18n::tr_static("  No experimental features available for now"),
            );
        }
...
fn experimental_popup_hint_line(keymap: &ListKeymap) -> Line<'static> {
    ...
    crate::i18n::tr_line(Line::from(spans))
}
