...
impl CommandPopup {
    ...
    fn filtered(&self) -> Vec<(CommandItem, Option<Vec<usize>>)> {
        ...
        let mut localized_alias: Vec<(CommandItem, Option<Vec<usize>>)> = Vec::new();
        ...
        for command in self.commands.iter() {
            ...
            let display_lower = display.to_lowercase();
            let standard_match =
                display_lower == filter_lower || display_lower.starts_with(&filter_lower);
            if standard_match {
                push_match(command.clone(), display, None, 0);
            } else if crate::i18n::command_matches_zh_alias(display, filter) {
                localized_alias.push((command.clone(), None));
            }
        }

        out.extend(exact);
        out.extend(prefix);
        out.extend(localized_alias);
        out
    }
