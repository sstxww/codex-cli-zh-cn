...
fn localize_selection_item(item: &mut SelectionItem) {
    let original_name = item.name.clone();
    item.name = crate::i18n::tr_owned(&item.name);
    item.description = item.description.take().map(crate::i18n::tr_owned);
    item.selected_description = item.selected_description.take().map(crate::i18n::tr_owned);
    item.disabled_reason = item.disabled_reason.take().map(crate::i18n::tr_owned);

    // Preserve canonical search terms while adding the translated display
    // text, so both English and Chinese queries continue to work.
    if let Some(search_value) = item.search_value.as_mut() {
        let mut additions = Vec::new();
        if item.name != original_name {
            additions.push(item.name.as_str());
        }
        if let Some(description) = item.description.as_deref()
            && !search_value.contains(description)
        {
            additions.push(description);
        }
        if !additions.is_empty() {
            search_value.push(' ');
            search_value.push_str(&additions.join(" "));
        }
    }
}

fn localize_selection_params(params: &mut SelectionViewParams) {
    if !crate::i18n::is_zh_cn() {
        return;
    }

    params.title = params.title.take().map(crate::i18n::tr_owned);
    params.subtitle = params.subtitle.take().map(crate::i18n::tr_owned);
    params.footer_note = params.footer_note.take().map(crate::i18n::tr_line);
    params.footer_hint = params.footer_hint.take().map(crate::i18n::tr_line);
    for (_, hint) in &mut params.tab_footer_hints {
        *hint = crate::i18n::tr_line(std::mem::take(hint));
    }
    params.search_placeholder = params.search_placeholder.take().map(crate::i18n::tr_owned);

    for item in &mut params.items {
        localize_selection_item(item);
    }
    for tab in &mut params.tabs {
        tab.label = crate::i18n::tr_owned(&tab.label);
...
impl ListSelectionView {
    ...
    pub fn new(
        ...
        localize_selection_params(&mut params);
