...
impl Renderable for MultiSelectPicker {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if search_area.height >= 2 {
            ...
            Line::from(crate::i18n::tr_static(SEARCH_PLACEHOLDER).dim())
            ...
            let query_span = if self.search_query.is_empty() {
                crate::i18n::tr_static(SEARCH_PLACEHOLDER).dim()
            } else {
                self.search_query.clone().into()
            };
        ...
        if list_area.height > 0 {
            ...
            render_rows_single_line(
                ...
                crate::i18n::tr_static("no matches"),
            );
        }
...
impl MultiSelectPickerBuilder {
    ...
    pub fn build(mut self) -> MultiSelectPicker {
        if crate::i18n::is_zh_cn() {
            self.title = crate::i18n::tr_owned(&self.title);
            self.subtitle = self.subtitle.take().map(crate::i18n::tr_owned);
        ...
        let instructions = crate::i18n::tr_line(Line::from(instructions)).spans;
