...
impl HooksBrowserView {
    ...
    fn event_header_lines() -> Vec<Line<'static>> {
        vec![
            crate::i18n::tr_static("Hooks").bold().into(),
            crate::i18n::tr_static("Lifecycle hooks from config and enabled plugins.")
                .dim()
                .into(),
        ]
    }
    ...
    fn handler_header_lines(
        ...
        let mut lines = vec![
            crate::i18n::tr_owned(format!("{} hooks", event_label(event_name)))
                .bold()
                .into(),
        ];
    ...
    fn event_table_lines(&self) -> Vec<Line<'static>> {
        ...
        let mut header = vec![
            display_padded(crate::i18n::tr_static("Event"), EVENT_COLUMN_WIDTH).into(),
            display_padded(crate::i18n::tr_static("Installed"), COUNT_COLUMN_WIDTH).into(),
            display_padded(crate::i18n::tr_static("Active"), COUNT_COLUMN_WIDTH).into(),
        ];
        if show_review {
            header
                .push(display_padded(crate::i18n::tr_static("Review"), COUNT_COLUMN_WIDTH).into());
        }
        header.push(crate::i18n::tr_static("Description").into());
        lines.push(Line::from(header));
        for (idx, row) in rows.into_iter().enumerate() {
            let selected = self.state.selected_idx == Some(idx);
            let needs_review = row.needs_review > 0;
            let mut row_line = vec![
                Span::from(display_padded(
    ...
    fn event_issue_lines(&self) -> Vec<Line<'static>> {
        ...
        lines.push(crate::i18n::tr_static("Issues").bold().into());
        lines.extend(
            self.entry
                .warnings
                .iter()
        ...
        self.handlers_for_event(event_name)
            ...
            .map(|(idx, hook)| {
                ...
                let row = match hook.trust_status {
                    HookTrustStatus::Modified => format!(
                        "[{marker}] {} · {}",
                        hook_title(idx),
                        crate::i18n::tr_static("modified")
                    ),
                    HookTrustStatus::Untrusted => format!(
                        "[{marker}] {} · {}",
                        hook_title(idx),
                        crate::i18n::tr_static("new")
                    ),
    ...
    fn detail_lines(&self, event_name: HookEventName, width: usize) -> Vec<Line<'static>> {
        let Some(hook) = self.selected_hook(event_name) else {
            return vec![
                crate::i18n::tr_static("No hooks installed for this event.")
                    .dim()
                    .into(),
            ];
        };
        ...
        match &hook.handler {
            HookHandlerMetadata::Command { command, r#async } => {
                ...
                let execution_mode = if *r#async {
                    crate::i18n::tr_static("Async")
                } else {
                    crate::i18n::tr_static("Sync")
                };
            ...
            HookHandlerMetadata::Prompt {} => {
                lines.push(detail_line("Handler", crate::i18n::tr_static("Prompt")));
            }
            HookHandlerMetadata::Agent {} => {
                lines.push(detail_line("Handler", crate::i18n::tr_static("Agent")));
            }
        }
        ...
        if let Some(limit) = hook.additional_context_limit {
            let value = if limit == 0 {
                crate::i18n::tr_owned("unlimited")
            } else {
                crate::i18n::tr_owned(format!("limit: {limit} approximate tokens"))
            };
    ...
    fn render_footer(&self, area: Rect, buf: &mut Buffer) {
        ...
        crate::i18n::tr_line(footer).dim().render(hint_area, buf);
    }
}
...
fn review_needed_message(count: usize) -> Option<String> {
    match count {
        0 => None,
        1 => Some(crate::i18n::tr_owned(
            "1 hook needs review before it can run.",
        )),
        count => Some(crate::i18n::tr_owned(format!(
            "{count} hooks need review before they can run."
        ))),
    }
}
...
fn hook_trust_label(status: HookTrustStatus) -> &'static str {
    crate::i18n::tr_static(match status {
...
fn event_description(event_name: HookEventName) -> &'static str {
    crate::i18n::tr_static(match event_name {
...
fn hook_source_summary(hook: &HookMetadata) -> String {
    match hook.source {
        HookSource::Plugin => hook
            .plugin_id
            .as_deref()
            .map(|plugin_id| format!("{} - {plugin_id}", crate::i18n::tr_static("Plugin")))
            .unwrap_or_else(|| crate::i18n::tr_owned("Plugin")),
...
fn config_source_label(source: HookSource) -> &'static str {
    crate::i18n::tr_static(match source {
...
fn detail_prefix(label: &str) -> String {
    format!("{} ", display_padded(crate::i18n::tr(label).as_ref(), 9))
}

fn detail_line(label: &str, value: &str) -> Line<'static> {
    Line::from(vec![detail_prefix(label).into(), value.to_string().dim()])
}
...
fn detail_wrapped_lines(
    ...
    let prefix = detail_prefix(label);
