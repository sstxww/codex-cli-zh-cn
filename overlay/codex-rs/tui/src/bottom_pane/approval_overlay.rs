...
impl ApprovalOverlay {
    ...
    fn build_options(
        ...
        let header = Box::new(ColumnRenderable::with([
            Line::from(crate::i18n::tr_owned(title).bold()).into(),
            Line::from("").into(),
            header,
        ]));
    ...
    match request {
        ApprovalRequest::Exec(request) => {
            ...
            if let Some(environment_id) = &request.environment_id {
                header.push(Line::from(vec![
                    crate::i18n::tr_static("Environment: ").into(),
            ...
            {
                header.push(Line::from(vec![
                    crate::i18n::tr_static("Permission rule: ").into(),
                    rule_line.cyan(),
                ]));
                header.push(Line::from(""));
            }
        ...
        ApprovalRequest::McpElicitation(request) => {
            ...
            lines.extend([
                Line::from(vec![
                    crate::i18n::tr_static("Server: ").into(),
