...
impl ChatWidget {
    ...
    fn model_menu_header(&self, title: &str, subtitle: &str) -> Box<dyn Renderable> {
        let title = crate::i18n::tr_owned(title);
        let subtitle = crate::i18n::tr_owned(subtitle);
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        Some(Line::from(crate::i18n::tr_owned(warning).red()))
    }
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        header.push(Line::from(
            crate::i18n::tr_owned(format!("Select Reasoning Level for {model_slug}")).bold(),
        ));
    ...
    pub(crate) fn open_advanced_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let mut header = ColumnRenderable::new();
        header.push(Line::from(
            crate::i18n::tr_static("Advanced Reasoning").bold(),
        ));
        header.push(Line::from(
            crate::i18n::tr_static("⚠ Consumes usage limits faster").cyan(),
        ));
        self.bottom_pane.show_selection_view(SelectionViewParams {
            header: Box::new(header),
            footer_hint: Some(standard_popup_hint_line()),
