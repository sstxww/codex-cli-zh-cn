...
impl ChatWidget {
    ...
    pub(crate) fn open_marketplace_add_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            crate::i18n::tr_owned("Add marketplace"),
            crate::i18n::tr_owned("owner/repo, git URL, or local marketplace path"),
            String::new(),
            Some(crate::i18n::tr_owned(
    ...
    fn plugin_install_auth_popup_params(&self) -> Option<SelectionViewParams> {
        ...
        let status_label = if is_installed {
            crate::i18n::tr_static("Already installed in this session.")
        } else {
            crate::i18n::tr_static("Install the required Apps in ChatGPT to continue:")
        };
        let mut header = ColumnRenderable::new();
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("{} plugin installed.", flow.plugin_display_name)).bold(),
        ));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("App setup {current}/{total}: {}", app.name)).dim(),
        ));
