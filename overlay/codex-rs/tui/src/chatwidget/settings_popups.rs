...
impl ChatWidget {
    ...
    pub(crate) fn open_personality_popup(&mut self) {
        if !self.is_session_configured() {
            self.add_info_message(
                crate::i18n::tr_owned("Personality selection is disabled until startup completes."),
                /*hint*/ None,
            );
            return;
        }
        if !self.current_model_supports_personality() {
            ...
            self.add_error_message(crate::i18n::tr_owned(format!(
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from(
            crate::i18n::tr_static("Select Personality").bold(),
        ));
        header.push(Line::from(
            crate::i18n::tr_static("Choose a communication style for Codex.").dim(),
        ));
