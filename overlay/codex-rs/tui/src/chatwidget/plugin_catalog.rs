...
impl Renderable for DelayedLoadingHeader {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        lines.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        ...
        if elapsed < LOADING_ANIMATION_DELAY {
            ...
            lines.push(Line::from(
                crate::i18n::tr(self.loading_text.as_str())
                    .into_owned()
                    .dim(),
            ));
            ...
            let loading_text = crate::i18n::tr(self.loading_text.as_str());
        ...
        if let Some(note) = &self.note {
            lines.push(Line::from(
                crate::i18n::tr(note.as_str()).into_owned().dim(),
            ));
        }
...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("Remove {marketplace_display_name} marketplace?")).dim(),
        ));
        header.push(Line::from(
            crate::i18n::tr_static("This removes the configured marketplace from Codex.").dim(),
        ));
    ...
    pub(super) fn marketplace_remove_loading_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("Removing {marketplace_display_name}...")).dim(),
        ));
    ...
    pub(super) fn plugin_install_loading_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("Installing {plugin_display_name}...")).dim(),
        ));
    ...
    pub(super) fn plugin_uninstall_loading_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_owned(format!("Uninstalling {plugin_display_name}...")).dim(),
        ));
    ...
    pub(super) fn plugins_error_popup_params(&self, err: &str) -> SelectionViewParams {
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_static("Failed to load plugins.").dim(),
        ));
    ...
    pub(super) fn marketplace_add_error_popup_params(&self) -> SelectionViewParams {
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_static("Failed to add marketplace.").dim(),
        ));
    ...
    pub(super) fn marketplace_remove_error_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_static("Failed to remove marketplace.").dim(),
        ));
    ...
    pub(super) fn plugin_detail_error_popup_params(
        ...
        header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
        header.push(Line::from(
            crate::i18n::tr_static("Failed to load plugin details.").dim(),
        ));
    ...
    pub(super) fn plugins_popup_params(
        ...
        let name_column_width = all_entries
            ...
            .chain([UnicodeWidthStr::width(crate::i18n::tr_static(
                "Add marketplace",
            ))])
            .max();
    ...
    pub(super) fn plugin_detail_popup_params(
        ...
        if !plugin.summary.installed {
            header.push(PluginDisclosureLine {
                line: Line::from(vec![
                    crate::i18n::tr_static("Data shared with this app is subject to the app's ")
                        .into(),
                    crate::i18n::tr_static("terms of service").bold(),
                    crate::i18n::tr_static(" and ").into(),
                    crate::i18n::tr_static("privacy policy").bold(),
                    ". ".into(),
                    crate::i18n::tr_static("Learn more").cyan().underlined(),
                    ".".into(),
                ]),
            });
        }
    ...
    fn plugin_selection_items<'a>(
        ...
        let status_label_width = plugin_entries
            .iter()
            .map(|(_, plugin, _)| UnicodeWidthStr::width(plugin_status_label(plugin)))
            .max()
            .unwrap_or(0);
        ...
        for (marketplace, plugin, display_name) in plugin_entries {
            ...
            let can_toggle_plugin = plugin.installed
                ...
                && !disabled_by_admin;
            let selected_status_label = display_padded(status_label, status_label_width);
...
pub(super) fn plugins_header(subtitle: String, count_line: String) -> Box<dyn Renderable> {
    let mut header = ColumnRenderable::new();
    header.push(Line::from(crate::i18n::tr_static("Plugins").bold()));
    header.push(Line::from(crate::i18n::tr_owned(subtitle).dim()));
    header.push(Line::from(crate::i18n::tr_owned(count_line).dim()));
    Box::new(header)
}
...
fn plugin_detail_status_label(plugin: &PluginSummary) -> &'static str {
    let label = if plugin.availability == PluginAvailability::DisabledByAdmin {
        "Disabled by admin"
    } else if plugin.install_policy == PluginInstallPolicy::InstalledByDefault {
        if plugin.installed {
            "Installed by admin"
        } else {
            "Enabled by Admin"
...
fn plugin_source_summary(plugin: &PluginDetail) -> String {
    match &plugin.summary.source {
        PluginSource::Local { .. } => crate::i18n::tr_owned("Local"),
        ...
        PluginSource::Remote => {
            ...
            format!("{} · {marketplace_label}", crate::i18n::tr_static("Remote"))
        }
    }
}

fn plugin_auth_policy_summary(auth_policy: PluginAuthPolicy) -> String {
    crate::i18n::tr_owned(match auth_policy {
        PluginAuthPolicy::OnInstall => "Auth on install",
        PluginAuthPolicy::OnUse => "Auth on use",
    })
}
...
fn plugin_version_summary(plugin: &PluginSummary) -> Option<String> {
    ...
    if let Some(local_version) = plugin.local_version.as_deref() {
        parts.push(if crate::i18n::is_zh_cn() {
            format!("本地 {local_version}")
        } else {
            format!("local {local_version}")
        });
    }
    ...
    {
        parts.push(if crate::i18n::is_zh_cn() {
            format!("远程 {remote_version}")
        } else {
            format!("remote {remote_version}")
        });
    }
...
fn plugin_share_discoverability_label(discoverability: PluginShareDiscoverability) -> &'static str {
    crate::i18n::tr_static(match discoverability {
...
fn plugin_share_creator_summary(context: &PluginShareContext) -> Option<String> {
    match (
        ...
        (Some(name), Some(account_id)) => Some(if crate::i18n::is_zh_cn() {
            format!("创建者 {name}（{account_id}）")
        } else {
            format!("creator {name} ({account_id})")
        }),
        (Some(name), None) => Some(if crate::i18n::is_zh_cn() {
            format!("创建者 {name}")
        } else {
            format!("creator {name}")
        }),
        (None, Some(account_id)) => Some(if crate::i18n::is_zh_cn() {
            format!("创建者账户 {account_id}")
        } else {
            format!("creator account {account_id}")
        }),
        (None, None) => None,
    }
}
...
fn plugin_share_principals_summary(principals: &[PluginSharePrincipal]) -> String {
    match principals.len() {
        0 => crate::i18n::tr_owned("No explicit principals"),
        1 if crate::i18n::is_zh_cn() => format!("1 个共享对象：{}", principals[0].name),
        1 => format!("1 principal: {}", principals[0].name),
        count if crate::i18n::is_zh_cn() => format!("{count} 个共享对象"),
...
fn plugin_brief_description(
    ...
    let status_label = plugin_status_label(plugin);
    let status_label = display_padded(status_label, status_label_width);
...
fn plugin_status_label(plugin: &PluginSummary) -> &'static str {
    let label = if plugin.availability == PluginAvailability::DisabledByAdmin {
        "Disabled"
    } else if !plugin.installed && plugin.install_policy == PluginInstallPolicy::InstalledByDefault
    {
        "Admin assigned"
    } else if plugin.installed {
        if plugin.enabled {
...
fn display_padded(value: &str, width: usize) -> String {
    format!(
        "{value}{}",
