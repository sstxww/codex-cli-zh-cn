# Localization-fork instructions

When a user gives an agent this repository to install or localize Codex, read `AGENT_START_HERE.md` first. Ask which components and language they want, inspect read-only, present exact changes and rollback, and wait for explicit confirmation. Never read or publish credentials and never merge separate `CODEX_HOME` directories.

For localization maintenance, preserve official `codex` behavior and keep `codex-zh` as a separate binary. Translate fixed UI text only; keep canonical slash commands, flags, paths, IDs, model responses, and raw tool output unchanged. Read `LOCALIZATION_PLAYBOOK.md` before adding another locale.
